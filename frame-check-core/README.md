...
|# CAM-15| **From eval** | `df.eval('new = A + B')` | `df.eval('C = A * 2', inplace=True)` | String expression assignment |
|# CAM-16| **From query results** | `df['new'] = df.query('A > 0')['B']` | Various query-based assignments | Indirect assignment |
|# CAM-17| **Conditional assignment** | `df.loc[condition, 'col'] = value` | `df.loc[df['A'] > 0, 'B'] = 1` | Partial column updates |
|# CAM-18| **Copy from another column** | `df['new'] = df['existing'].copy()` | `df['B'] = df['A'].copy()` | Explicit copy |
|# CAM-19| **Transform operations** | `df['new'] = df['col'].apply(func)` | `df['B'] = df['A'].apply(lambda x: x*2)` | Via transformations |
|# CAM-20| **Expanding columns** | `df[['A','B']] = df['combined'].str.split(expand=True)` | String split to multiple cols | Creates multiple columns |
|# CAM-21| **From groupby** | `df['new'] = df.groupby('A')['B'].transform('mean')` | Groupby aggregations | Broadcast results back |
|# CAM-22| **From pivot** | Pivot operations can create new columns | `df.pivot(columns='A')` | Reshaping creates columns |
|# CAM-23| **From unstack** | `df = df.unstack()` | Multi-index to columns | Index level to columns |
|# CAM-24| **From pd.get_dummies** | `df = pd.get_dummies(df, columns=['A'])` | One-hot encoding | Creates multiple columns |
|# CAM-25| **Update method** | `df.update(other_df)` | `df.update(df2[['A']])` | Updates existing columns |
