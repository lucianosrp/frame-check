...
def _print_code_line(
    ...
    if code_line is not None:
        ...
        print(f"{' ' * line_width}|{' ' * relative_col}{underline_char * underline_length}")
    else:
...
def _print_data_source_note(diag: "Diagnostic", lines: list[str], line_width: int) -> None:
    ...
    _print_gutter(line_width)
    _print_code_line(diag.data_source_location, lines, 
                     len(_get_line(lines, diag.data_source_location[0]) or ""), "~", line_width)
